  ...
  <div class="col-span-full sm:col-span-6 xl:col-span-4 bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700">
    <div class="flex flex-col h-full">
      ...
      <div class="grow p-5">
        <div class="flex justify-between items-start">
          ...
          <header>
            <div class="flex mb-2">
              ...
              <div class="mt-1 pr-1">
                <router-link class="inline-flex text-slate-800 dark:text-slate-100 hover:text-slate-900 dark:hover:text-white" to="#">
                  <h2 class="text-xl leading-snug justify-center font-semibold">{{item.warehouseName}}</h2>
                </router-link>
                <div class="flex items-center"><span class="text-sm font-medium text-slate-400 -mt-0.5 mr-1">-&gt;</span> <span>{{item.warehouseLocation}}</span></div>
              </div>
            </div>
          </header>
          <!-- Menu button -->
        ...
        <!-- Bio -->
        <div class="mt-2">
          <div class="text-sm">总容量：{{item.totalCapacity}}</div>
        </div>
      </div>
    </div>
  </div>
