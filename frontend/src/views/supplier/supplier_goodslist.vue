    ...
    <div class="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
      ...
      <main class="grow dark:bg-slate-900 p-4">
        ...
        <div>
          ...
          <el-button type="success" @click="filterOnshelf('all')">所有商品</el-button>
        </div> 
        <!-- Search bar -->
        ...
        <el-table :data="paginatedGoods" border stripe style="width: 100%" max-height="66vh" v-loading="loading"
          ...
          <el-table-column prop="onShelf" label="销售状态" width="180" fixed="right">
            <template #default="{ row }">
    
              <el-tag :type="getTagType(row.onShelf)">{{ getStatusText(row.onShelf) }}</el-tag>
            </template>
          </el-table-column>
  ...
  setup() {
    ...
    const filteredGoods = computed(() => {
      ...
      if (filterValue.value === 'all') {
        ...
        return filtered.filter(good => good.onShelf === filterValue.value);
      }
      
    })
    const filterOnshelf = (shelf) => {
  filterValue.value = shelf;
